...
BEGIN
	SELECT 	@vAmount = abs(TRL_PRICE * (CASE WHEN TRL_ORIGQTY IS NOT NULL 
			...
			@vTraCode = TRA_CODE,
			@vTrlEvent = EVT_CODE,
			@vReference2 = CONCAT('RETURN:', CONVERT(NVARCHAR(2), TRL_DATE, 101)),
	...
	BEGIN
			SET @vReference4 = CONCAT(@vTrlEvent, '-', @vPart, '-', @vDateFormat)
