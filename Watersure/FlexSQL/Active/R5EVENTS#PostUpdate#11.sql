...
BEGIN
	...
	SELECT @countClosComm = COALESCE(COUNT(ADD_CODE) ,0)
	FROM R5ADDETAILS
...
IF (@jbtype = 'EMIN'
	AND @stsold IN ('INPR','PLAN')
	AND @stsnew = 'CHRV'
	AND (@changeSts = 'Implementation' OR @changeSts = 'Change Request'))
		BEGIN --For Eng Investigations to be able to go through the process twice, this may soon be deprecated with a new process 13/04/2020
			SET @newChangeSts = 'Change Review (4 Pillars)'
			SET @changeReject = 1
		END
IF(@stsold='CHWA' AND @stsnew = 'PLAN') --Will email change owner, if owner is a department it will email a manager
	BEGIN
		...
		IF @owner = 'PROJ' SET @email = 'AMASSARO'
...
IF (@stsnew in ('C') AND @stsold NOT IN ('C') AND @jbtype NOT LIKE 'MEC' 
			AND (@action IS NULL OR @comp IS NULL OR @woStart IS NULL OR @countClosComm < 1))
...
IF (@stsnew in ('CLOS') AND @dept = 'OPS' AND @jbtype IN ('BRKD','CAL','CAOH','CARP','CASV','CMBD','CMPR','PMCA','PMCM','PMIN','PMOH','PMRP','PMSV') AND @woStart IS NOT NULL AND @action IS NOT NULL AND @comp IS NOT NULL AND @countClosComm > 0)
