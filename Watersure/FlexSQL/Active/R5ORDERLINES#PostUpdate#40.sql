...
BEGIN
  SELECT 
   ...
   WHERE  ORL_SQLIDENTITY = :rowid 
    AND (ORL_RECVQTY = 0 OR ORL_RECVQTY IS NULL); --Not receipted Lines
  ...
  IF @snew IS NOT NULL
    BEGIN
      ...
      IF @evnt IS NULL AND @dept IS NULL
        BEGIN
        ...
        WHERE ORL_SQLIDENTITY = :rowid
        AND COALESCE(ORL_UDFCHAR07,'x') <> @dept;  
      ...
      IF 
        ( @snew  = 'A' AND 
          (     (@costcheck < 1 OR  @cost IS NULL) OR
                ( (@deptcheck < 1 AND @cost NOT LIKE '0%') OR 
                  (@dept IS NULL AND @cost NOT LIKE '0%')))
        )   
          BEGIN
