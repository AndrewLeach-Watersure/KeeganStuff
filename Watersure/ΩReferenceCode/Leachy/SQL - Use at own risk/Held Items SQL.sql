...
LEFT OUTER JOIN
        (
         SELECT   SUM(TST_QTY) INTRANSITQTY, 
                  TST_TOCODE 
         FROM     R5TRANSTOCK 
         WHERE    TST_PART = :partcode AND 
                  TST_PART_ORG = :partorg AND 
                  TST_ORDER IS NULL 
         GROUP BY TST_TOCODE
...
LEFT OUTER JOIN 
       (
        ...
        WHERE     REQ_CODE = RQL_REQ AND 
                  ...
                  RQL_ACTIVE = '+' AND 
                  REQ_RTYPE  = 'STST' AND 
                  RQL_PART = :partcode  AND 
                  RQL_PART_ORG = :partorg AND 
                  NOT EXISTS (
                              SELECT 'x' 
                              FROM   R5TRANSACTIONS  
...
LEFT OUTER JOIN
       (
        ...
        WHERE PIC_RSTATUS = 'A' AND 
              ...
              PIP_PART = :partcode AND 
              PIP_PART_ORG = :partorg AND 
...
LEFT OUTER JOIN
       (
        SELECT SUM(RES_QTY) RESQTY, 
               ...
               RES_STORE 
        FROM   R5RESERVATIONS 
        WHERE  RES_PART = :partcode AND 
               RES_PART_ORG = :partorg 
        GROUP BY RES_STORE) reservations ON STO_STORE = reservations.RES_STORE,
R5STORES

WHERE   (
        STO_PART = :partcode OR 
        (COALESCE(par_trackbycondition ,'-') = '+' AND par_parentpart = :partcode)
        )
AND STO_PART_ORG = :partorg 
