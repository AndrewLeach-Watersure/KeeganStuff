...
Ext.create("Ext.window.Window", {
  title: "Count of open work orders per WO Type",
...
var chart = new Chart(ctx, {
  type: 'bar',
  data: {
    labels: data.map((el) => el.jobtype_desc),
    datasets: [{
      label: 'Open',
      data: data.map((el) => el._open),
      backgroundColor: 'rgba(255, 99, 132, 0.2)',
      borderColor: 'rgba(255, 99, 132, 1)',
      borderWidth: 1
    }, {
      label: 'Closed',
      data: data.map((el) => el.closed),
  ...
  options: {
    ...
    onClick: (e) => {
      ...
      const dataY = chart.scales["y-axis-0"].getValueForPixel(canvasPosition.y);
      console.log(dataY);
